...
class TemplatesController extends Controller
{
  ...
  public static function post(Request $request)
  {
    ...
    if ($request->input('done')) {
      Template::find($request->input('done'))->update(['decanat_check ' => 1])->save();
    ...
    if ($request->input('noShow')) {
      Template::find($request->input('noShow'))->update(['decanat_check ' => 0])->save();
      return redirect('/center/shablon_prikazy');
      // $connect->query("UPDATE Practices.templates SET decanat_check = 0, comment = '' WHERE id =". $_GET['noShow'] .";");
    }
    if ($request->input('remake')) {
      Template::find($request->input('remake'))->update(['decanat_check ' => 2, 'comment' => $request->input('comment')])->save();
      return redirect('/center/shablon_prikazy');
