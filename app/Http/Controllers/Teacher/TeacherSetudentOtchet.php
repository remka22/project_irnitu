...
class TeacherSetudentOtchet extends Controller
{
    public static function get(Request $request)
    {
        ...
        if($request->get('check')){
            $student_practics = StudentPractic::whereIn('teacher_id', $arr_id_tscore)->with('company', 'student.group.stream', 'student.otchet')->get();
        }
        else{
            $student_practics = StudentPractic::where('status', "<>", 2)->whereIn('teacher_id', $arr_id_tscore)->with('company', 'student.group.stream', 'student.otchet')->get();
        }
        dd($student_practics);
